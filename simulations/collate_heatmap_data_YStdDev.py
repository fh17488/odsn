...
def main():
    ...
    title = topology+"_heatmap_N1000_YStdDev"
    data_directory = "../Results/opinion_dynamics/"
    outfile = title+".csv"
    ...
    for pe_i in range(pe_steps):
        for u_i in range(u_steps):
            pe_val = pe[pe_i]
            u_val = u[u_i]
            infile = data_directory + topology + "_network_" + str(pe_val) + "_"  + str(u_val) + "_MetricYStdDevLog.txt"
            print(infile)
